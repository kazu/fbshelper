...
/*
must call 1 times per Symbol ;
go run github . com / cheekybits / genny gen "Symbol=Root False=false" ;
*/
...
func (node Symbol) SearchInfo(pos int, fn base.RecFn, condFn base.CondFn) {
	...
	// g := Symbol_IdxToTypeGroup[i]
	//     if node.IsLeafAt(i) {
	//         fInfo := base.Info(node.ValueInfo(i))
	//         if condFn(pos, fInfo) {
	// fn(base.NodePath{Name: "Symbol", Idx: i}, fInfo)
	//         }
	//         continue
	//     }
...
func (node Symbol) Info() (info base.Info) {
	...
	// 	size := 0
	// 	for i :=0; i < node.CountOfField(); i++ {
	// size += base.TypeToSize[Symbol_IdxToType[i]]
	// 	}
	// 	info.Size = size
	// 	return info
	// }
...
func (node Symbol) ValueInfo(i int) base.ValueInfo {
	...
	// 	for i :=0; i < node.CountOfField(); i++ {
	// 		info.Pos += info.Size
	// info.Size = base.TypeToSize[Symbol_IdxToType[i]]
	// 		node.ValueInfos = append(node.ValueInfos,  info)
	// 	}
	// }

	// grp := Symbol_IdxToTypeGroup[i]
...
// func (node Symbol) SizeAsStruct() int {
// if base.FalseName["Symbol"] {
// 		size := 0
// 		for i :=0; i < node.CountOfField(); i++ {
// size += base.TypeToSize[Symbol_IdxToType[i]]

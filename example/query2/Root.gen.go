...
/*
must call 1 times per Root ;
go run github . com / cheekybits / genny gen "Root=Root False=false" ;
*/
...
func (node Root) Info() (info base.Info) {
	...
	// 	size := 0
	// 	for i :=0; i < node.CountOfField(); i++ {
	// size += base.TypeToSize[Root_IdxToType[i]]
	// 	}
	// 	info.Size = size
	// 	return info
	// }
...
func (node Root) ValueInfo(i int) base.ValueInfo {
	...
	// 	for i :=0; i < node.CountOfField(); i++ {
	// 		info.Pos += info.Size
	// info.Size = base.TypeToSize[Root_IdxToType[i]]
	// 		node.ValueInfos = append(node.ValueInfos,  info)
	// 	}
	// }

	// grp := Root_IdxToTypeGroup[i]
...
// func (node Root) SizeAsStruct() int {
// if base.FalseName["Root"] {
// 		size := 0
// 		for i :=0; i < node.CountOfField(); i++ {
// size += base.TypeToSize[Root_IdxToType[i]]
